...
def detect_events(video_file, user_info: Dict) -> List[Dict]:
    ...
    if check_server_connection():
        ...
        if task_id:
            ...
            # 업로드 실패
            st.error("⚠️ 서버 업로드에 실패했습니다.")
            return []
    else:
        # 서버가 없으면 빈 리스트 반환
        st.warning("ℹ️ 서버에 연결할 수 없습니다. 서버를 실행해주세요.")
        return []


def generate_llm_content_via_api(
    event_type: str, 
    violation_type: str, 
    ...
    try:
        ...
        if event_details:
            data["event_details"] = json.dumps(event_details, ensure_ascii=False)
...
def format_report_data_via_api(event: Dict, user_info: Dict) -> Dict:
    """
    서버 API를 통해 신고 양식 데이터 생성 (실패 시 클라이언트 측 fallback)
    ...
    try:
        ...
        # API 실패 시 클라이언트 측에서 포맷팅 (fallback)
        time_str = event.get("time", "")
        if " " in time_str:
            time_str = time_str.split(" ")[1]
        
        return {
            "0. 위반 이벤트 유형": event.get("event_type", ""),  # 0번 항목 추가
...
def render_llm_option():
    ...
    llm_provider = st.selectbox(
        "LLM 제공자 선택",
        ["사용 안 함", "OpenAI", "Anthropic", "Google"],
        index=0,
        help="제목 및 신고 내용을 LLM으로 자동 생성할 LLM 제공자를 선택하세요."
    )
    
    api_key = None
    if llm_provider != "사용 안 함":
        api_key = st.text_input(
            f"{llm_provider} API 키",
            type="password",
...
def render_event_details(event: Dict, user_info: Dict, llm_provider: str, api_key: Optional[str]):
    """개별 이벤트 상세 정보 렌더링 (아코디언 방식)"""
    
    # LLM으로 제목/내용 생성 (필요시)
    if llm_provider != "사용 안 함" and api_key:
        llm_content = generate_llm_content_via_api(
            event["event_type"], 
    ...
    with st.expander(
        ...
        with tab3:
            ...
            report_data = format_report_data_via_api(event, user_info)
...
def main():
    ...
    with st.sidebar:
        st.header("⚙️ 설정")
        llm_provider, api_key = render_llm_option()
        st.markdown("---")
    ...
    # LLM 정보 추가
    user_info["llm_provider"] = llm_provider
    user_info["llm_api_key"] = api_key
    ...
    if 'events' in st.session_state and st.session_state.get('video_uploaded', False):
        ...
        if not filtered_events:
            ...
            for event in filtered_events:
                render_event_details(event, user_info, llm_provider, api_key)
                st.markdown("---")
