...
# -*- coding: utf-8 -*-
"""
YOLOv26s Vehicle Tracking
- Fine-tuned vehicle detection 모델로 tracking 수행
- 이미지 시퀀스를 시간별로 그룹화하여 tracking
"""
...
def select_sequence(sequences):
    ...
    while True:
        try:
            choice = input(f"\n추적할 시퀀스 번호를 입력하세요 (1-{len(sorted_keys)}): ")
            idx = int(choice) - 1
...
def run_tracking(model, images, sequence_name):
    """
    선택한 시퀀스로 tracking 수행
    ...
    """
    print("\n" + "=" * 70)
    print(f"Tracking 시작: {sequence_name}")
    print("=" * 70)
    ...
    # 저장 경로 구성: track/yolo26s_{VERSION}/{seq_name}
    ...
    print()
    print(f"📁 결과 저장 위치:")
    print(f"   - 이미지: {TRACK_PROJECT}/{TRACK_NAME_PREFIX}/{sequence_name}/")
    print(f"   - 텍스트: {TRACK_PROJECT}/{TRACK_NAME_PREFIX}/{sequence_name}/labels/")
    print("=" * 70)
...
def main():
    """메인 실행 함수"""
    print("=" * 70)
    print("YOLOv26s Vehicle Tracking")
    print("=" * 70)
    print()
    ...
    # Step 4: Tracking 수행
    run_tracking(model, images, timestamp)
