...
# 모드 선택
MODE = "val"  # "val" 또는 "predict"
# - "val": Validation 모드 (GT + Prediction 시각화, 평가 메트릭 계산)
# - "predict": Prediction 모드 (Prediction만 시각화, 빠름)
...
def create_temp_dataset_yaml(images, sequence_name):
    """
    임시 데이터셋 YAML 파일 생성 (VAL 모드용)
    ...
    """
    # 임시 디렉토리 생성
    temp_dir = Path(f"temp_dataset_{sequence_name}")
    ...
    for img_path in images:
        ...
        # 이미지 링크
        link_img = temp_images_dir / img_path.name
        if not link_img.exists():
            link_img.symlink_to(img_path.absolute())
        
        # 라벨 링크 (val/labels에서 찾기)
        label_path = Path(str(img_path).replace('/images/', '/labels/').replace('.jpg', '.txt'))
        if label_path.exists():
            link_label = temp_labels_dir / label_path.name
            if not link_label.exists():
                link_label.symlink_to(label_path.absolute())
    
    # YAML 파일 생성
    yaml_content = {
        'path': str(temp_dir.absolute()),
        'train': 'images',  # 사용하지 않지만 필수
        'val': 'images',
        'names': {0: 'vehicle'}
    }
...
def run_inference_predict(model, images, sequence_name):
    """
    PREDICT 모드: Prediction만 시각화
    """
    print(f"모드: PREDICT (Prediction만 시각화)")
    print()
    ...
    return frame_count, total_detections, detections_per_frame, project_path, sequence_name


def run_inference_val(model, images, sequence_name):
    """
    VAL 모드: GT + Prediction 시각화, 평가 메트릭 계산
    """
    print(f"모드: VALIDATION (GT + Prediction 시각화, 평가 메트릭)")
    print()
    
    # 임시 데이터셋 YAML 생성
    print("  임시 데이터셋 생성 중...")
    yaml_path = create_temp_dataset_yaml(images, sequence_name)
    print(f"  ✓ 데이터셋 생성 완료: {yaml_path}")
    print()
    ...
    return frame_count, metrics, project_path, sequence_name
...
def run_inference(model, images, sequence_name):
    ...
    # 모드에 따라 다른 함수 호출
    if MODE.lower() == "val":
        frame_count, metrics, project_path, save_name = run_inference_val(model, images, sequence_name)
        ...
        print()
        print(f"📁 결과 저장 위치:")
        print(f"   - GT 시각화: {project_path}/{save_name}/val_batch*_labels.jpg")
        print(f"   - Prediction 시각화: {project_path}/{save_name}/val_batch*_pred.jpg")
        print(f"   - 전체 결과: {project_path}/{save_name}/")
        
    else:  # predict
        frame_count, total_detections, detections_per_frame, project_path, save_name = run_inference_predict(model, images, sequence_name)
        ...
        print()
        print(f"📁 결과 저장 위치:")
        print(f"   - 이미지: {project_path}/{save_name}/")
        print(f"   - 텍스트: {project_path}/{save_name}/labels/")
